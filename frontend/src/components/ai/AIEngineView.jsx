...
import {
  Sparkles, BrainCircuit, Send, Bot, User, Flame, Thermometer, Snowflake,
...
function AIChat() {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: "Hi, I'm Internite AI. Ask me anything about your CRM and pipeline." }
  ])
  ...
  const composer = (
    <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-2 shadow-[0_10px_32px_rgba(47,151,242,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80">
      <textarea
        value={input}
        onChange={(e) => setInput(e.target.value)}
        onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send() } }}
        placeholder="Ask Internite AI anything..."
        rows={1}
        className="w-full resize-none border-none bg-transparent px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 outline-none dark:text-slate-100"
      />
      <div className="flex items-center justify-between gap-2 px-1 pb-1 pt-1">
        <div className="flex items-center gap-1.5">
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800" aria-label="Attach">
            <Paperclip className="h-4 w-4" />
          </button>
          <span className="hidden items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 sm:inline-flex">
        ...
        <button
          ...
          aria-label="Send"
          className="inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-slate-900 text-white transition-transform hover:scale-105 disabled:opacity-40 disabled:hover:scale-100 dark:bg-white dark:text-slate-900"
        >
  ...
  if (isIdle) {
    return (
      <div className="flex min-h-[420px] flex-col items-center justify-center gap-6 px-4 py-10 text-center sm:px-8">
        <div className="inline-flex items-center gap-2 rounded-full border border-brand-200 bg-brand-50 px-3.5 py-1.5 text-xs font-semibold text-brand-600 dark:border-brand-800 dark:bg-brand-950/30 dark:text-brand-300">
          <BrainCircuit className="h-3.5 w-3.5" />
          Internite AI Assistant
        </div>
        <h2 className="max-w-lg text-2xl font-bold leading-tight text-slate-900 dark:text-slate-50 sm:text-3xl">
          Turn your <span className="text-brand-500">pipeline</span> into <span className="text-brand-500">answers</span>
        </h2>
        <div className="w-full max-w-xl">{composer}</div>
        <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-slate-200/80 bg-white/70 text-left dark:border-slate-700/70 dark:bg-slate-900/60">
          {QUICK.map((q, i) => (
            <button
              key={q}
              onClick={() => send(q)}
              className={`flex w-full items-center gap-3 px-4 py-3 text-sm text-slate-600 transition-colors hover:bg-brand-50/70 dark:text-slate-300 dark:hover:bg-brand-950/20 ${i !== 0 ? 'border-t border-slate-100 dark:border-slate-800' : ''}`}
            >
              <MessageSquare className="h-3.5 w-3.5 flex-shrink-0 text-brand-400" />
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2"><BrainCircuit className="w-6 h-6 text-brand-500" /> Bhadawar</h1>
